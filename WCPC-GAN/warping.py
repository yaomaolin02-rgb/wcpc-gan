...
class WarpingGAN(nn.Module):
    ...
    def forward(self, input):
        ...
        concate1 = torch.cat((splitinput, globalfeature, grid), dim=2).transpose(1, 2).reshape(batch_size,
                                                                                               int(512 / self.numgrid) + self.dimofgrid + 512,
                                                                                               2048)
        after_folding1 = self.mlp1(concate1)
        concate2 = torch.cat(
            (splitinput, globalfeature, after_folding1.reshape(batch_size, 3, self.numgrid, self.m).transpose(1, 2)),
            dim=2).transpose(1, 2).reshape(batch_size, int(512 / self.numgrid) + 3 + 512, 2048)
